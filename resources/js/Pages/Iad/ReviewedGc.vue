<template>
    <AuthenticatedLayout>
        <a-card>
            <a-input-search allow-clear enter-button placeholder="Input search here..." v-model:value="reviewGcSearch"
                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'details'">
                        <a-button type="primary"
                            @click="() => $inertia.get(route('iad.reviewed.gc.details', { id: record.spexgc_id }))">
                            <template #icon>
                                <FileSearchOutlined />
                            </template>
                        </a-button>
